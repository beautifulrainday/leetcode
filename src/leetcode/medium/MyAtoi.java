...
public class MyAtoi {
    ...
    public static int myAtoi(String s) {
        ...
        for (int i = 0; i < s.length(); i++) {
            ...
            if (i == 0 && "-".equalsIgnoreCase(ch.toString())) {
                flg = 1;
                continue;
            }
            if (i == 0 && "+".equalsIgnoreCase(ch.toString())) {
        ...
        s = sb.toString();
        s = "".equalsIgnoreCase(s) ? "0" : s;
        try {
            num = flg == 0 ? Integer.parseInt(s) : -1 * Integer.parseInt(s);
        } catch (NumberFormatException e) {
            if (flg == 1) {
                num = new Double(Math.pow(-2, 31)).intValue();
            } else {
                Double tempDouble = Math.pow(2, 31) - 1;
                num = tempDouble.intValue();
            }
        }
        return num;
    }
    ...
    public static void main(String[] args) {
        ...
        //System.out.println(-1 * new Double(Math.pow(2, 31)).intValue());
