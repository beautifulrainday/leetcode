...
public class UniqueEmailAddresses {
    ...
    public static void main(String[] args) {
        //String[] emails = {"fg.r.u.uzjo.pw@kziczvh.com"};
    ...
    public static int numUniqueEmails(String[] emails) {
        List<String> s = Arrays.asList(emails);
        Set<String> results = new HashSet<>();
        for (int i = 0; i < s.size(); i++) {
            String local = s.get(i).split("@")[0].replace(".", "").split("\\+")[0];
            String dest = s.get(i).split("@")[1];
            String result = local + "@" + dest;
            results.add(result);
        }
        return results.size();
    }
